...
> **Round 2 + 3 verification:** every finding re-verified live on-device across
> two fix rounds; per-finding `status` + `v2_/v3_evidence` and notes are in
> `findings.json`, screenshots in `shots/v2_*.png` / `shots/v3_*.png`.
> Final statuses: **FIXED** F-1, F-2, F-3 (v3: active tab accent-blue/white,
> palette pinned at `lv.STATE.CHECKED`), F-4 (v3: no more opacity-dim in mpe.py;
> flat muted disabled sliders — residual nit: disabled stepper/Edit *buttons*
> keep a faint khaki tint), F-5, F-6, F-7, F-8 (v3: duplicates numbered
> "brush1 (2)"), F-10 (v3: footer clamps to "+ 14 members", matching the status
> line), F-11 (v3: Delete renders disabled for protected system modules), F-13,
> F-15 · **NO-CHANGE-NEEDED** F-9 (starred = bright yellow, verified) ·
> **NOT-FIXED / deferred** F-12, F-14, F-16 (out of scope this round).
